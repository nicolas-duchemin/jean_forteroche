...
class ModelPost extends Model
{
    ...
    {
        $db = $this->dbConnect();
        $postsData = $db->query('SELECT post_id, title, SUBSTRING(content, 1, 450) AS preview, DATE_FORMAT(creation_date, \'%d/%m/%Y à %Hh%imin%ss\') AS creation_date_fr, DATE_FORMAT(update_date, \'%d/%m/%Y à %Hh%imin%ss\') AS update_date_fr FROM posts ORDER BY creation_date DESC');
    ...
    {
        $db = $this->dbConnect();
        $lastPostData = $db->query('SELECT post_id, title, SUBSTRING(content, 1, 450) AS preview, DATE_FORMAT(creation_date, \'%d/%m/%Y à %Hh%imin%ss\') AS creation_date_fr, DATE_FORMAT(update_date, \'%d/%m/%Y à %Hh%imin%ss\') AS update_date_fr FROM posts ORDER BY creation_date DESC LIMIT 0, 1');
