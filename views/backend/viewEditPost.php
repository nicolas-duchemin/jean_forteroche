...
ob_start(); 
    ...
    <form action="index.php?action=editPost&amp;postId=<?= $post['post_id'] ?>" method="post">
        <div class="flex">
            <p><label for="title">Titre</label><input type="text" id="title" name="title" value="<?= strtoupper(htmlspecialchars($post['title'])) ?>"></p>
            <p class="date">
        ...
        <p class="buttonArea"><input class="button" type="submit" value="Enregistrer la modification"> <a class="button" href="index.php?action=seeRemovePost&amp;postId=<?= $post['post_id'] ?>">Supprimer le chapitre</a></p>
    </form>
    ...
    while ($comment = $commentsData->fetch()) {
        ...
        <form action="index.php?action=editComment&amp;commentId=<?= $comment['comment_id'] ?>&amp;postId=<?= $comment['post_id'] ?>" method="post">
            <div class="flex">
                <p><label for="author">Auteur</label><input type="text" id="author" name="author" value="<?= htmlspecialchars($comment['author']) ?>"></p>
                <div class="flex">
